...
For my ColorPalette kernel, I’m using a basic array of Color objects along with an integer that keeps track of how many colors are currently in the palette. I went with an array because it’s simple, fast, and works well for keeping a list of colors in order.

The convention is basically the rulebook for when the data inside is valid. In this case, it just means that count should always be between 0 and the length of the array, and only the first count spots in the array actually hold colors. Anything after that doesn’t matter and isn’t used.

The correspondence is how the inside of the code lines up with the abstract idea of a color palette. So the actual “palette” we think about matches the colors in the array from index 0 up to count - 1. For example, if count is 3, the palette’s colors are whatever’s stored in array[0], array[1], and array[2].
